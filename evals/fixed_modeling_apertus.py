...
class ApertusAttention(nn.Module):
    ...
    def __init__(self, config: ApertusConfig, layer_idx: int | None = None):
        ...
        if getattr(config, "qk_norm", False):
            self.q_norm = ApertusRMSNorm(config.hidden_size // config.num_attention_heads, eps=config.rms_norm_eps)
            self.k_norm = ApertusRMSNorm(config.hidden_size // config.num_attention_heads, eps=config.rms_norm_eps)
        else:
            self.q_norm = None
            self.k_norm = None
